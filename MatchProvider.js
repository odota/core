var fs = require("fs"),
    ...
    var MatchProvider = function(user, pass, authcode) {
        this.ready = false;
        ...
        // node-steam's logOn now requires an object that is a valid protobuf payload so we must omit authCode or shaSentryFile if ours are empty.
        var logOnDetails = {
            "accountName": user,
            "password": pass
        },
        ...
        this.bot.on("loggedOn", onSteamLogOn).on('sentry', onSteamSentry).on('servers', onSteamServers).on('error', onSteamError);
}
