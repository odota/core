...
stream.on('data', (match) => {
  ...
  const newRating1 = currRating1 + Math.floor(kFactor * (win1 - e1));
  const newRating2 = currRating2 + Math.floor(kFactor * (win2 - e2));
  teams[match.team_id1] = newRating1;
  teams[match.team_id2] = newRating2;
