...
* Data volume
  ...
  * 3 matches a day rule
    * We found that users typically play around 3 matches a day.  This number decreases as you scale to thousands of users because of match overlap.
      * Use this to help you with capacity planning.
* Reddit posts/traffic spikes
  * We got 1000 users from the first post[https://www.reddit.com/r/DotA2/comments/2sp595/introducing_yasp_a_free_opensource_stats_website/] in January 2015.  We were only expecting 100 or so.
...
* Scaling
  * Start small, grow with your userbase!
    * Don't try to import every match right away.  It'll cost you a ton of money and you probably don't need most of it at first.
  * Use a timeout/inactivity check
...
* Config
  * Don't put secrets in your code!  It's really easy to accidentally push them to GitHub, and then you have to deal with rotating them.
...
* Microservices
  ...
  * Scale individually.  If you find that you're CPU-bound on one worker, you can easily deploy multiple instances of it to scale out.
* Keep backups!
...
* Cloud vs dedicated
  ...
  * Storage is expensive!  Cloud providers typically give you free data ingress, then charge you to store the data or take it out.  Keep this in mind before committing a large chunk of data to the cloud.
  * Avoid hosted services.  It's way cheaper to run Redis on a VM than it is to pay for hosted Redis, etc.
...
* SQL vs NoSQL
  * MongoDB
    ...
    * Probably could have gotten it to scale if we spent the time to figure it out, but wanted to try something new
  * Postgres
    * + SQL, people know it!
...
* Donation model
  * Be prepared for good and bad months.
    ...
    * Use a subscription model if you want a steadier revenue stream.
...
* Don't try to build your own solution to everything.
  * Use Steam OAuth, don't try to roll your own auth.  You'll need account_ids anyway so you can link them to the data you're getting from the Steam API.
...
* Goals
  * Raise Bus number[https://en.wikipedia.org/wiki/Bus_factor]
    * The minimum number of people that could be hit by a bus to bring down your project.
  ...
  * Open source/Open access to data
    * We think programming is cool and want to teach as many people who are interested how to do it/work with Dota data.
    * We also want to make all the data we have accessible and machine-readable.
    * Lets people build cool things based on our work/data

Maybe @albertcui can help with pictures and graphics idk
