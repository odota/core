...
sudo docker run -d --name=gcdata --restart=always --net=host --log-opt max-size=1g -e PROVIDER=gce -e ROLE=gcdata -e RETRIEVER_HOST=localhost odota/core:latest sh -c "npm start"

# If already initialized
sudo docker start retriever
sudo docker start gcdata
