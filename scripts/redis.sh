...
sudo docker run -d --name redis --restart=always -v /redis.conf:/usr/local/etc/redis/redis.conf -v /var/lib/redis:/var/lib/redis --net=host redis:3 /usr/local/bin/redis-server /usr/local/etc/redis/redis.conf
