...
sudo docker run -d --restart=always --net=host -e FRONTEND_PORT=80 -e PROVIDER=gce -e ROLE=web yasp/yasp:latest sh -c "node deploy.js ----max-old-space-size=512"
'
...
do
    sudo docker run -d --restart=always -e PROVIDER=gce -e ROLE=parser yasp/yasp:latest sh -c "node deploy.js ----max-old-space-size=512"
done
'
