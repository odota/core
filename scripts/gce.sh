...
sudo docker run -d --name parserHost --net=host yasp/parser
...
gcloud compute instance-templates create task-1 --preemptible --machine-type n1-highcpu-16 --image-family ubuntu-1404-lts --image-project ubuntu-os-cloud --boot-disk-size 50GB --boot-disk-type pd-ssd --metadata startup-script='#!/bin/bash
curl -sSL https://get.docker.com/ | sh
