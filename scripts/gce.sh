...
#cassandra
gcloud compute --project "peaceful-parity-87002" disks create "disk-cassandra-3" --size "2000" --zone "us-central1-b" --type "pd-standard"
gcloud compute instances delete -q cassandra-3
gcloud compute instances create cassandra-3 --machine-type n1-highmem-4 --image container-vm --boot-disk-size 10GB --boot-disk-type pd-ssd --disk "name=disk-cassandra-3,device-name=persistent-disk-1" 
gcloud compute instances add-metadata cassandra-3 --metadata-from-file startup-script=./scripts/cassandra.sh
