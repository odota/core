...
async function start() {
    ...
    while(true) {
        ...
        const result = await cassandra.execute("select match_id, player_slot, stuns, token(match_id) from player_matches where token(match_id) >= ? and player_slot = 1 limit 500 ALLOW FILTERING;", [randomBigint.toString()], {
            prepare: true,
            fetchSize: 500,
            autoPage: true,
          });
        ...
        const ids = result.rows.filter(result => result.stuns == null && result.match_id < limit).map(result => result.match_id);
