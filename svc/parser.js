...
function runParse(match, job, cb) {
  ...
  cp.exec(`curl --max-time 180 ${url} | ${url && url.slice(-3) === 'bz2' ? 'bunzip2' : 'cat'} | curl -X POST --data-binary @- ${config.PARSER_HOST} | node processors/createParsedDataBlob.js ${match.match_id} ${Boolean(match.doLogParse)}`,
