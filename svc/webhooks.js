...
const { redisCount, filterStream, spyStream } = require('../util/utility');
...
function createWebhookFilter(match) {
  return (webhook) => {
...
function createWebhookCaller(match) {
...
const readFromFeed = async (seqNum) => {
  ...
  result[0][1].forEach(async (dataArray) => {
    ...
    queries.getWebhooks(db)
      .pipe(filterStream(createWebhookFilter(match)))
      .pipe(spyStream(() => redisCount(redisClient, 'webhook')))
      .pipe(spyStream(createWebhookCaller(match)));
  });
