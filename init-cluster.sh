...
sudo docker run -d --name yasp_DEPLOY --restart=always -e VIRTUAL_HOST=yasp.co --expose=5000 yasp/yasp:latest "node deploy.js"
sudo docker stop yasp
sudo docker rm -v yasp
sudo docker rename yasp_DEPLOY yasp