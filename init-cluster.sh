...
sudo docker run -d --net=host yasp/yasp:latest "node deploy.js && sleep infinity"
...
sudo docker run -d --net=host yasp/yasp:latest "./node_modules/pm2/bin/pm2 start parser.js -i 0 && sleep infinity"
'
