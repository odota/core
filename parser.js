...
function runParse(match, cb)
{
    ...
    function preprocessEventBuffer()
    {
        var preTypes = {
            "combat_log": function(e)
            {
                ...
                if (e.subtype === "DOTA_COMBATLOG_GAME_STATE" && e.value === 5)
                {
                    game_zero = e.time;
                }
            },
    ...
    function processReduce()
    {
        ...
        {
            var e = entries[i];
            var identifier = [e.player_slot, e.type, e.subtype, e.targethero, e.targetillusion, e.key].join(":");
            e.value = e.value || 1;
        ...
        for (var i = 0; i < entries.length; i++)
        {
            ...
            for (var j = 0; j < teamfights.length; j++)
            {
                ...
                if (e.time >= tf.start && e.time <= tf.end)
                {
                    ...
                    else if (e.subtype === "DOTA_COMBATLOG_BUYBACK")
                    {
                        //bought back
                    ...
                    else if (e.subtype === "DOTA_COMBATLOG_DAMAGE")
                    {
                        //sum damage
                    ...
                    else if (e.subtype === "DOTA_COMBATLOG_GOLD" || e.subtype === "DOTA_COMBATLOG_XP")
                    {
                        ...
                        if (tf.players[e.slot])
                        {
                            ...
                            tf.players[e.slot][types[e.subtype]] += e.value;
                        }
                    }
                    else if (e.subtype === "DOTA_COMBATLOG_ABILITY" || e.subtype === "DOTA_COMBATLOG_ITEM")
                    {
                        //count skills, items
                        populate(e, tf);
                    }
