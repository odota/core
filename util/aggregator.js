...
const isSignificant = utility.isSignificant;
const getAggs = utility.getAggs;
...
function aggregator(matches, fields, existing)
{
  ...
  for (let i = 0; i < matches2.length; i++)
    ...
    const significant = isSignificant(pm);
    for (var key in fields)
        {
      if (significant)
            {
...
{
  const teammates = aggData.teammates;
  const player_win = isRadiant(m) === m.radiant_win;
  const group = m.heroes ||
    {};
  for (const key in group)
