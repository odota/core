...
var hero_ids = Object.keys(constants.heroes);

module.exports = function(db)
{
    matches.get('/:player_id/:match_id/:info?', function(req, res, cb)
    {
        ...
        db.first("personaname")
            ...
            .asCallback(function(err, p) {
                if (err)
                {
                    return cb(err);
                }

                var mChance = new Chance(req.params.player_id, req.params.match_id);

                var startTime = mChance.date();
                startTime.setFullYear(2017);

                var match = {};
                match.match_id = req.params.match_id;
                match.skill = mChance.weighted([null, 1, 2, 3], [.1,.3, .3, .3]);
                ...
                function generatePlayers() {
                    var players = [];
                    var heroes = mChance.pickset(hero_ids, 10);
                    for (var i = 0; i < 10; i++) {
                        var player = {
                        ...
                        var gold = [], lh = [], xp = [];
                        var lastGold = 0, lastLH = 0, lastXP = 0;
                        ...
                        for (j = 0; j < mChance.weighted([0, 1, 2, 3, 4, 5, 6], [.01, .04, .5, .1, .2, .3, .3]); j++) {
                ...
                function generateItemTimeline()
                {
                    var items = [];
                    var lastTime = -60;
                    for (var j = 0; j < mChance.natural({max: 40}); j++) {
                ...
                function generateWards() {
                    var wards = [];
                    for (var j = 0; j < mChance.natural({max: 20}); j++) {
                        wards.push({
