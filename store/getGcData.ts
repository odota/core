...
export default async function getGcData(match: GcDataJob): Promise<{ match_id: number, cluster: number, replay_salt: number }> {
  const matchId = match.match_id;
  if (!matchId || Number.isNaN(Number(matchId)) || Number(matchId) <= 0) {
    throw new Error('invalid match_id');
  }
  // Check if we have it saved already
  ...
  // Extract match_id, cluster, replay_salt
  const gcdata = row?.gcdata ? JSON.parse(row?.gcdata) : null;
  if (config.NODE_ENV !== 'development' && gcdata && gcdata.match_id && gcdata.cluster && gcdata.replay_salt) {
