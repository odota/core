...
const { redisCount, convert64to32, serialize, deserialize, isRadiant, isContributor, countItemPopularity, averageMedal, } = utility;
...
function getMatchBenchmarks(m, cb) {
    async.map(m.players, (p, cb) => {
        ...
        async.eachSeries(Object.keys(benchmarks), (metric, cb) => {
            ...
            let key = [
                'benchmarks',
                utility.getStartOfBlockMinutes(config.BENCHMARK_RETENTION_MINUTES, -1),
                metric,
                p.hero_id,
            ].join(':');
            const backupKey = [
...
function getHeroItemPopularity(db, redis, heroId, options, cb) {
  ...
  `, [heroId || 0]).asCallback((err, purchaseLogs) => {
        ...
        const startGameItems = countItemPopularity(items.filter((item) => item.time <= 0 && item.cost <= 600));
        const earlyGameItems = countItemPopularity(items.filter((item) => item.time > 0 && item.time < 60 * 10 && item.cost >= 500));
        const midGameItems = countItemPopularity(items.filter((item) => item.time >= 60 * 10 && item.time < 60 * 25 && item.cost >= 1000));
        const lateGameItems = countItemPopularity(items.filter((item) => item.time >= 60 * 25 && item.cost >= 2000));
        return cb(null, {
...
function getPlayerMatches(accountId, queryObj, cb) {
    ...
    `, queryObj.project
            ...
            .join(','));
        const matches = [];
        return cassandra.eachRow(query, [accountId], {
            prepare: true,
            fetchSize: 5000,
            autoPage: true,
        }, (n, row) => {
...
function getProPeers(db, input, player, cb) {
    ...
    const teammates = input;
    return db
        .raw(`select *, notable_players.account_id
...
function upsert(db, table, row, conflict, cb) {
    cleanRowPostgres(db, table, row, (err, row) => {
        ...
        const query = util.format('INSERT INTO %s (%s) VALUES (%s) ON CONFLICT (%s) DO UPDATE SET %s', table, Object.keys(row).join(','), values.join(','), Object.keys(conflict).join(','), update.join(','));
        return db
            .raw(query, Object.keys(row).map((key) => row[key]))
            .asCallback(cb);
    });
}
    ...
    async.series({
        ...
        scr(cb) {
            if (row.solo_competitive_rank) {
                upsert(db, 'solo_competitive_rank', {
                    account_id: row.account_id,
                    rating: row.solo_competitive_rank,
                }, { account_id: row.account_id }, cb);
            }
            else {
                cb();
            }
...
function writeCache(accountId, cache, cb) {
    return async.each(cache.raw, (match, cb) => {
        cleanRowCassandra(cassandra, 'player_caches', match, (err, cleanedMatch) => {
            ...
            const query = util.format('INSERT INTO player_caches (%s) VALUES (%s)', Object.keys(serializedMatch).join(','), Object.keys(serializedMatch)
                .map(() => '?')
                .join(','));
...
function insertMatch(match, options, cb) {
    ...
    async function upsertMatchPostgres(cb) {
        ...
        const result = match.leagueid
            ? await db.raw(`select leagueid from leagues where leagueid = ? and (tier = 'premium' OR tier = 'professional')`, [match.leagueid])
            : null;
    ...
    function upsertMatchCassandra(cb) {
        ...
        return cleanRowCassandra(cassandra, 'matches', match, (err, match) => {
            ...
            const query = util.format('INSERT INTO matches (%s) VALUES (%s)', Object.keys(obj).join(','), Object.keys(obj)
                .map(() => '?')
                .join(','));
            ...
            return cassandra.execute(query, arr, {
                ...
                return async.each(players || [], (pm, cb) => {
                    ...
                    cleanRowCassandra(cassandra, 'player_matches', pm, (err, pm) => {
                        ...
                        const query2 = util.format('INSERT INTO player_matches (%s) VALUES (%s)', Object.keys(obj2).join(','), Object.keys(obj2)
                            .map(() => '?')
                            .join(','));
                        const arr2 = Object.keys(obj2).map((k) => obj2[k] === 'true' || obj2[k] === 'false'
                            ? JSON.parse(obj2[k])
                            : obj2[k]);
...
function getTeamScenarios(req, cb) {
    ...
    db.raw(`SELECT scenario, is_radiant, region, sum(games) games, sum(wins) wins
     ...
     LIMIT 1000`, { scenario }).asCallback((err, result) => cb(err, result));
}
